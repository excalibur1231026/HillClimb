...
void ScrollBarView::initVertical()
{
    ...
    Sprite *back_sprite = Sprite::create("scrollbar-bg.png");
    addChild(back_sprite);
    // Create 9-patch sprite
    Rect imageSize = Rect(0, 0, 21, BAR_MIN_SIZE);
    Rect insets    = Rect(0, 4, 21, 12);
    bar = Scale9Sprite::create("scroll-bar.png", imageSize, insets);
    bar->setScaleX(0.8);
    
    back_sprite->setAnchorPoint(Vec2(0.5f, 0));
    ...
    {
        this->setPosition(position.x  - BAR_FIX_SIZE - 20/* + size.width*/, position.y);
        back_sprite->setPosition(Vec2(bar->getContentSize().width / 2 - 1, 0));
        int h =view->getViewSize().height;
        back_sprite->setScaleY(h / back_sprite->getContentSize().height);
    }
}
